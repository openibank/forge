...
BASE_FILES=$(find dist/apps/forge-ide-e2e/src/tests -type f \( -name "*.test.js" -o -name "*.spec.js" \) -print0 \
...
for TESTFILE in $TESTFILES; do
    ...
    attempt=0
    while true; do
      if npx nightwatch --config dist/apps/forge-ide-e2e/nightwatch-${1}.js dist/apps/forge-ide-e2e/src/tests/${TESTFILE}.js --env=$1; then
        # success
        break
      fi
      # failure
