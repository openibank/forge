...
if [ ! -d "dist/apps/forge-ide-e2e/src/tests" ]; then
...
if [ -f "failed-basenames.txt" ] && [ -s "failed-basenames.txt" ]; then
  ...
  if [ -n "${CIRCLECI_TOKEN:-}" ]; then
    ...
    FAILED_BASENAMES=$(node scripts/circleci-failed-tests.js --slug ${CIRCLECI_PROJECT_SLUG:-gh/openibank/forge} --workflow "$WORKFLOW_NAME" --branch "${CIRCLE_BRANCH:-}" --jobs "forge-ide-browser" --limit 1 || true)
  else
...
cat reports/failed/files.txt | ./apps/forge-ide/ci/setup_slither_if_needed.sh
...
for TESTFILE in $(cat reports/failed/files.txt); do
  ...
  attempt=0
  while true; do
    if npx nightwatch --config dist/apps/forge-ide-e2e/nightwatch-${BROWSER}.js dist/apps/forge-ide-e2e/src/tests/${TESTFILE}.js --env=$BROWSER; then
      break
    fi
