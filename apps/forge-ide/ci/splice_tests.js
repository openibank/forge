...
let cmd = `grep -IRiL "@disabled" "dist/apps/forge-ide-e2e/src/tests"`
...
exec(cmd, (error, stdout, stderr) => {
  ...
  let files = stdout.split('\n').filter(f => f.includes('.test')).map(f => f.replace('dist/apps/forge-ide-e2e/src/tests/', '')).map(f => f.replace('.js', ''))
