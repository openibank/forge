...
import type { AppLifecycle } from '@creditchain/forge-lib'
import { RemixNavigator } from './types'
import { Profile } from '@remixproject/plugin-utils'

function isForgeAppManagerDebugEnabled(): boolean {
  try {
    return localStorage.getItem('forge-app-manager-debug') === 'true'
  } catch {
    return false
  }
}

function logForgeAppManager(...args: any[]): void {
  if (isForgeAppManagerDebugEnabled()) console.log(...args)
}

function errorForgeAppManager(...args: any[]): void {
  if (isForgeAppManagerDebugEnabled()) console.error(...args)
}
...
class BaseForgeAppManager extends PluginManager {
...
export class ForgeAppManager extends BaseForgeAppManager {
  actives = []
  ...
  constructor() {
    ...
    this.pluginsDirectory = 'https://raw.githubusercontent.com/openibank/forge-plugins-directory/main/build/metadata.json'
    ...
    if (this.canDeactivatePlugin(from, to)) {
      if (profile.methods.includes('deactivate')) {
        try {
          ...
          logForgeAppManager(e)
        }
      }
      await this.toggleActive(name)
    } else {
      logForgeAppManager('cannot deactivate', name)
    }
  }
