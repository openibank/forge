...
import { SolidityUnitTesting } from '@creditchain/forge-ui/solidity-unit-testing' // eslint-disable-line
import { TestTabLogic } from '@creditchain/forge-ui/solidity-unit-testing' // eslint-disable-line

import { ViewPlugin } from '@remixproject/engine-web'
import helper from '../../lib/helper'
import { canUseWorker, urlFromVersion } from '@creditchain/forge-solidity'
import { PluginViewWrapper } from '@creditchain/forge-ui/helper'

var { UnitTestRunner, assertLibCode } = require('@creditchain/forge-tests')
...
export default class TestTab extends ViewPlugin {
  constructor (fileManager, offsetToLineColumnConverter, filePanel, compileTab, appManager, contentImport) {
    ...
    this.allFilesInvolved = ['.deps/forge-tests/remix_tests.sol', '.deps/forge-tests/remix_accounts.sol']
  ...
  async createTestLibs () {
    ...
    if (provider) {
      await provider.addExternal('.deps/forge-tests/remix_tests.sol', assertLibCode, 'remix_tests.sol')
      await provider.addExternal('.deps/forge-tests/remix_accounts.sol', this.testRunner.accountsLibCode, 'remix_accounts.sol')
    }
  }
