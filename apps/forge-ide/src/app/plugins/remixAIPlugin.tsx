...
import { trackMatomoEvent, Features, ChatPromptMetadata } from '@creditchain/forge-api'
import { remixAILogger, RemoteInferencer, IRemoteModel, IParams, GenerationParams, AssistantParams, CodeExplainAgent, SecurityAgent, CompletionParams, OllamaInferencer } from '@creditchain/forge-ai-core';
import { CodeCompletionAgent, ContractAgent, workspaceAgent, IContextType, mcpDefaultServersConfig, mcpBasicServersConfig, mcpWebSearchServersConfig } from '@creditchain/forge-ai-core';
import { MCPInferencer, DeepAgentInferencer, onApiKeysChange } from '@creditchain/forge-ai-core';
import { IMCPServer, IMCPConnectionStatus } from '@creditchain/forge-ai-core';
import { RemixMCPServer, createRemixMCPServer } from '@creditchain/forge-ai-core';
import { AIModel } from '@creditchain/forge-ai-core';
import { aiErrorFromException, parseAIErrorEnvelope } from '@creditchain/forge-ai-core';
...
export class RemixAIPlugin extends Plugin {
  ...
  async generate(prompt: string, params: IParams=AssistantParams, newThreadID:string="", useRag:boolean=false, statusCallback?: (status: string) => Promise<void>): Promise<any> {
    ...
    if (useRag) {
      statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.fetchingRAGContext'))
      try {
        let ragContext = ""
    ...
    await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.generatingNewWorkspace'))
    const result = await this.remoteInferencer.generate(userPrompt, params)

    await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.creatingContracts'))
  ...
  async generateWorkspace (userPrompt: string, params: IParams=AssistantParams, newThreadID:string="", useRag:boolean=false, statusCallback?: (status: string) => Promise<void>): Promise<any> {
    ...
    await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.performingWorkspaceRequest'))
    if (useRag) {
      await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.fetchingRAGContext'))
      try {
        let ragContext = ""
        const options = { headers: { 'Content-Type': 'application/json', } }
    ...
    await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.loadingWorkspaceContext'))
    ...
    await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.generatingWorkspaceUpdates'))
    const result = await this.remoteInferencer.generateWorkspace(userPrompt, params)

    await statusCallback?.(await this.getLocalizedMessage('forgeApp.ai.status.applyingChanges'))
