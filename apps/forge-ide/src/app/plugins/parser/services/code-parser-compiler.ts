...
import { CompilationResult, CompilationSource } from '@creditchain/forge-solidity'
import { CodeParser } from "../code-parser";
import { fileDecoration, fileDecorationType } from '@creditchain/forge-ui/file-decorators'
import { sourceMappingDecoder } from '@creditchain/forge-debug'
import { CompilerRetriggerMode, CompilationSourceCode } from '@creditchain/forge-solidity';
import { findLinesInStringWithMatch, SearchResultLine } from '@creditchain/forge-ui/search'
...
export default class CodeParserCompiler {
  ...
  init() {
    ...
    this.onAstFinished = async (success, data: CompilationResult, source: CompilationSourceCode, input: any, version) => {
      ...
      // cast from the remix-plugin interface to the solidity one. Should be fixed when remix-plugin move to the forge-project repository
