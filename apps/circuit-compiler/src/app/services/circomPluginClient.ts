...
import { extractNameFromKey, extractParentFromKey, CompilerReport } from '@creditchain/forge-ui/helper'
    ...
    if (pathExists) return path
      ...
      if (relativePathExists) return relativePath
        ...
        if (path.startsWith('circomlib')) {
          ...
          else throw new Error(`Report path ${path} does not exist in the Forge FileSystem`)
        } else {
          throw new Error(`Report path ${path} does not exist in the Forge FileSystem`)
        }
      }
    }
  }
