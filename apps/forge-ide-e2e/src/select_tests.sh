...
TESTFILES=( $(grep -IRiL "\'@disabled\': \?true" "dist/apps/forge-ide-e2e/src/tests" | sort ) )
...
do
    ...
    if [ "$opt" = "list" ]; then
        ...
        if [ "$BROWSER" = "firefoxDesktop" ]; then
            yarn run build:e2e && nightwatch --config dist/apps/forge-ide-e2e/nightwatch-firefox.js $opt --env=$BROWSER
        elif [ "$BROWSER" = "chrome" ]; then
            yarn run build:e2e && nightwatch --config dist/apps/forge-ide-e2e/nightwatch-chrome.js $opt --env=$BROWSER
