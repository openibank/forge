...
module.exports = {
  ...
  'run Remixd tests #group1': '' + function (browser: NightwatchBrowser) {
    browser.perform(async (done) => {
      try {
        ...
        killRemixdProcesses()
        remixd = await spawnRemixd(join(process.cwd(), '/apps/forge-ide', '/contracts'))
      } catch (err) {
        console.error(err)
        // fail
  ...
  'Should listen on compilation result from hardhat #group4': '' + function (browser: NightwatchBrowser) {
    ...
    browser.perform(async (done) => {
      try {
        // Proactively kill any hanging remixd processes before spawning a new one
        killRemixdProcesses()
        remixd = await spawnRemixd(join(process.cwd(), '/apps/forge-ide/hardhat-boilerplate'))
      } catch (err) {
        console.error(err)
        browser.assert.fail('Failed to start remixd')
      }
  ...
  'Should disable git when running remixd #group9': '' + function (browser: NightwatchBrowser) {
    ...
    browser.perform(async (done) => {
      try {
        remixd = await spawnRemixd(join(process.cwd(), '/apps/forge-ide', '/contracts/hardhat'))
      } catch (err) {
        console.error(err)
        browser.assert.fail('Failed to start remixd')
      }
...
async function setupHardhatProject(): Promise<void> {
  ...
  try {
    const server = spawn('git clone https://github.com/NomicFoundation/hardhat-boilerplate && cd hardhat-boilerplate && yarn install && yarn add "@typechain/ethers-v5@^10.1.0" && yarn add "@typechain/hardhat@^6.1.2" && yarn add "typechain@^8.1.0" && echo "END"', [], { cwd: process.cwd() + '/apps/forge-ide', shell: true, detached: true })
    return new Promise((resolve, reject) => {
...
async function compileHardhatProject(): Promise<void> {
  console.log(process.cwd())
  try {
    const server = spawn('npx hardhat compile', [], { cwd: process.cwd() + '/apps/forge-ide/hardhat-boilerplate', shell: true, detached: true })
    return new Promise((resolve, reject) => {
      server.stdout.on('data', function(data) {
        console.log('stdout: ' + data.toString())
      })
