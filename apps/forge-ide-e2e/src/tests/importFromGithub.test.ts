...
const testData = {
  validURL: 'https://github.com/openibank/forge.git',
  invalidURL: 'https://github.com/Oppelin/Roles.sol',
  JSON: 'https://github.com/openibank/forge.git'
}
...
module.exports = {
  ...
  'Clone From GitHub with Valid URL #group2': function (browser: NightwatchBrowser) {
    browser
      ...
      .waitForElementVisible({
        selector: `//*[@data-id='tab-active' and @data-path="forge.git/README.md"]`,
        locateStrategy: 'xpath'
      })
      .getEditorValue((content) => {
        browser.assert.ok(content.indexOf('Forge') !== -1, 'content contains Forge branding')
      })
  },
  'Confirm JSON After Cloning From GitHub For Valid URL #group2': function (browser: NightwatchBrowser) {
    browser
      ...
      .waitForElementVisible("*[data-path='forge.git/package.json'")
      .getEditorValue((content) => {
        browser.assert.ok(content.indexOf('"name": "forge",') !== -1, 'content contains Forge package name')
      })
      .end()
  }
}
