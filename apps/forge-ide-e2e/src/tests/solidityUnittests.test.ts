...
module.exports = {
  ...
  'Changing current path when workspace changed and checking test files creation #group4': function (browser: NightwatchBrowser) {
    browser
      ...
      .expandAllFolders()
      .waitForElementVisible('li[data-id="treeViewLitreeViewItem.deps/forge-tests/remix_tests.sol"]')
      .waitForElementVisible('li[data-id="treeViewLitreeViewItem.deps/forge-tests/remix_accounts.sol"]')
      .openFile('.deps/forge-tests/remix_tests.sol')
      // remix_test.sol should be opened in editor
      .getEditorValue((content) => browser.assert.ok(content.indexOf('library Assert {') !== -1))
      .openFile('.deps/forge-tests/remix_accounts.sol')
