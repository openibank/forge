...
for TESTFILE in $TESTFILES; do
    yarn run test --use-isogit --test ./build-e2e/forge-desktop/test/tests/app/${TESTFILE} || yarn run test --use-isogit --test ./build-e2e/forge-desktop/test/tests/app/${TESTFILE} || TEST_EXITCODE=1
done
