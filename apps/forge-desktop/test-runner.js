...
class DesktopTestRunner {
  constructor() {
    ...
    // Simple, direct paths - relative to current directory (apps/forge-desktop)
    this.sourceTestDir = 'test/tests/app';
    this.testDir = 'build-e2e/forge-desktop/test/tests/app';
  }
  ...
  getAvailableTests() {
    ...
    files.forEach((file, index) => {
      ...
      tests.push({
        ...
        name: testName,
        file: file,
        builtPath: `build-e2e/forge-desktop/test/tests/app/${file.replace('.ts', '.js')}`, // Correct path for yarn command
        exists: exists
      });
    });
