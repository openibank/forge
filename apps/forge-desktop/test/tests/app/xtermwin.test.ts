...
const tests = {
  ...
  'run a git clone command': function (browser: NightwatchBrowser) {
    browser
      .perform(function () {
        const actions = this.actions({ async: true })
        return actions.sendKeys('git clone https://github.com/openibank/forge').sendKeys(this.Keys.ENTER)
      })
      .waitForElementVisible('*[data-id="treeViewLitreeViewItemforge"]', 10000)
      .click('*[data-id="treeViewLitreeViewItemforge"]')
      .waitForElementVisible('*[data-id="treeViewLitreeViewItemforge/README.md"]', 10000)
  },
  'remove the cloned repo': function (browser: NightwatchBrowser) {
    browser
      ...
      .perform(function () {
        const actions = this.actions({ async: true })
        return actions.sendKeys('Remove-Item -Path forge -Recurse -Force').sendKeys(this.Keys.ENTER)
      })
      .waitForElementNotPresent('*[data-id="treeViewLitreeViewItemforge"]', 10000)
  },
