...
describe('testRunner: forge-tests CLI', function(){
  this.timeout(120000)
  // forge-tests binary, after build, is used as executable

  const executablePath = resolve(__dirname + '/../../../dist/libs/forge-tests/bin/forge-tests')

  const result = spawnSync('ls', { cwd: resolve(__dirname + '/../../../dist/libs/forge-tests') })
  if (result) {
    ...
    if (!dirContent.includes('node_modules')) {
      execSync('yarn add @creditchain/forge-lib ../../libs/forge-lib', { cwd: resolve(__dirname + '/../../../dist/libs/forge-tests') })
      execSync('yarn add @creditchain/forge-url-resolver ../../libs/forge-url-resolver', { cwd: resolve(__dirname + '/../../../dist/libs/forge-tests') })
      execSync('yarn add @creditchain/forge-solidity ../../libs/forge-solidity', { cwd: resolve(__dirname + '/../../../dist/libs/forge-tests') })
      execSync('yarn add @creditchain/forge-simulator ../../libs/forge-simulator', { cwd: resolve(__dirname + '/../../../dist/libs/forge-tests') })
      execSync('yarn install', { cwd: resolve(__dirname + '/../../../dist/libs/forge-tests') })
    }
  }

  describe('test various CLI options', function() {
    it('forge-tests version', () => {
    ...
    it('forge-tests help', () => {
      ...
      const expectedHelp = `Usage: forge-tests [options] [command] <file_path>
...
Commands:
  ...
  help                        output usage information`
    ...
    it('forge-tests running a test file', function() {
      const res = spawnSync(executablePath, [resolve(__dirname + '/examples_0/assert_ok_test.sol')])
      // match initial lines
      console.log(res.stdout.toString())
      expect(res.stdout.toString().trim()).to.match(/:: Running tests using forge-tests ::/)
    ...
    it('forge-tests running a test file with custom compiler version', () => {
    ...
    it('forge-tests running a test file with unavailable custom compiler version (should fail)', () => {
    ...
    it('forge-tests running a test file with custom EVM', () => {
    ...
    it('forge-tests running a test file by enabling optimization', () => {
    ...
    it('forge-tests running a test file by enabling optimization and setting runs', () => {
    ...
    it('forge-tests running a test file without enabling optimization and setting runs (should fail)', () => {
    ...
    it('forge-tests running a test file with all options', () => {
