## Remix Tests
[![npm version](https://badge.fury.io/js/%40creditchain%2Fforge-tests.svg)](https://www.npmjs.com/package/@creditchain/forge-tests)
[![npm](https://img.shields.io/npm/dt/@creditchain/forge-tests.svg?label=Total%20Downloads)](https://www.npmjs.com/package/@creditchain/forge-tests)
[![npm](https://img.shields.io/npm/dw/@creditchain/forge-tests.svg)](https://www.npmjs.com/package/@creditchain/forge-tests)
[![GitHub](https://img.shields.io/github/license/mashape/apistatus.svg)](https://github.com/openibank/forge/tree/master/libs/forge-tests)
...
`@creditchain/forge-tests` is a tool to test Solidity smart contracts. It works underneath Forge IDE "Solidity Unit Testing" plugin which is used to write and run test cases for a contract. Tests are written in Solidity itself. `@creditchain/forge-tests` can be used as CLI and a library too.

To know more about Forge IDE `Solidity Unit Testing Plugin`, visit [Forge IDE official documentation](https://forge-ide.readthedocs.io/en/latest/unittesting.html).

### Installation
* As a dev dependency:

`yarn add --dev @creditchain/forge-tests`

* As a global NPM module to use as CLI:

`yarn global add @creditchain/forge-tests`
...
### Test structure
`@creditchain/forge-tests` provides and injects a built-in assert library for testing purpose.
...
One can see all available option for forge-tests using `--help` command.

```
$ forge-tests --help           
Usage: forge-tests [options] [command]
...
```
$ forge-tests examples/
``` 
* To run single test file named `simple_storage_test.sol` inside `examples` directory
```
$ forge-tests examples/simple_storage_test.sol
```
**NOTE:** forge-tests will assume that name of tests file ends with `"_test.sol"`. e.g `simple_storage_test.sol`
...
```Javascript
pragma solidity >=0.4.22 <=0.8.0;
import "remix_tests.sol"; // injected by forge-tests
...
```
$ forge-tests simple_storage_test.sol

	👁	:: Running forge-tests - Unit testing for solidity ::	👁
...
Most of the `forge-tests` options are there to define a custom compiler context. With an extended custom compiler context, execution of above test file will go as:

```
$ forge-tests --compiler 0.7.4 --evm istanbul --optimize true --runs 300 simple_storage_test.sol
...
Import library:
```Javascript
const remixTests = require('@creditchain/forge-tests');
```
