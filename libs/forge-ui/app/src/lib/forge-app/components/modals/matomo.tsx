...
const MatomoDialog = (props: MatomoDialogProps) => {
  ...
  useEffect(() => {
    if (visible && showMatomo) {
      modal({
        id: 'matomoModal',
        title: <FormattedMessage id="forgeApp.matomoTitle" />,
        message: message(),
        okLabel: <FormattedMessage id="forgeApp.accept" />,
        okFn: handleAcceptAllClick,
        cancelLabel: <FormattedMessage id="forgeApp.managePreferences" />,
