...
import { AppContext, AppModal, useAuth } from '@creditchain/forge-ui/app'
import { DISCORD_URL, FORGE_WEBSITE_URL, FORGE_DOCS_URL, SUBSTACK_URL, TWITTER_URL } from '@creditchain/forge-ui/helper'
import { MatomoEvent, EditorEvent, AIEvent, Features } from '@creditchain/forge-api'
//@ts-ignore
import { TrackingContext } from '@creditchain/forge-ide/tracking'
import { ConsoleLogs, EventManager, QueryParams } from '@creditchain/forge-lib'
...
import { GenerationParams } from '@creditchain/forge-ai-core';
...
export const EditorUI = (props: EditorUIProps) => {
  ...
  \t\t\t\t\t\t\t\t${intl.formatMessage({ id: 'editor.importantLinks.text1' })}: ${FORGE_WEBSITE_URL}/\n
  \t\t\t\t\t\t\t\t${intl.formatMessage({ id: 'editor.importantLinks.text2' })}: ${FORGE_DOCS_URL}/\n
  \t\t\t\t\t\t\t\tGithub: https://github.com/openibank/forge\n
  \t\t\t\t\t\t\t\tDiscord: ${DISCORD_URL}\n
  \t\t\t\t\t\t\t\tCreditChain updates: ${SUBSTACK_URL}\n
  \t\t\t\t\t\t\t\tX: ${TWITTER_URL}\n
  `
  ...
  useEffect(() => {
    ...
    if (file.language === 'sol') {
      monacoRef.current.editor.setModelLanguage(file.model, 'forge-solidity')
  ...
  function handleEditorWillMount(monaco) {
    ...
    monacoRef.current.languages.register({ id: 'forge-solidity' })
    ...
    monacoRef.current.languages.setMonarchTokensProvider('forge-solidity', solidityTokensProvider as any)
    monacoRef.current.languages.setLanguageConfiguration('forge-solidity', solidityLanguageConfig as any)
    ...
    monacoRef.current.languages.registerDefinitionProvider('forge-solidity', new RemixDefinitionProvider(props, monaco))
    monacoRef.current.languages.registerDocumentHighlightProvider('forge-solidity', new RemixHighLightProvider(props, monaco))
    monacoRef.current.languages.registerReferenceProvider('forge-solidity', new RemixReferenceProvider(props, monaco))
    monacoRef.current.languages.registerHoverProvider('forge-solidity', new RemixHoverProvider(props, monaco))
    monacoRef.current.languages.registerCompletionItemProvider('forge-solidity', new RemixCompletionProvider(props, monaco))
    monacoRef.current.languages.registerInlineCompletionsProvider('forge-solidity', inlineCompletionProviderRef.current)
    monaco.languages.registerCodeActionProvider('forge-solidity', new RemixCodeActionProvider(props, monaco))
  ...
  return (
    <div className="w-100 h-100 d-flex flex-column-reverse">
      ...
      <DiffEditor
        originalLanguage={'forge-solidity'}
        modifiedLanguage={'forge-solidity'}
        original={''}
        modified={''}
